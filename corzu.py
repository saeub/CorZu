...
def main(mables_file,conll_file): 
    """
    Main loop
    test:
        ...
        for line in conll_file:
            if line=='\n':
                ...
                if csets_out==[] and mables_out==[]:
                    output_str += '\t'.join(line[0:-1])+'\t-\n'
                elif csets_out==[] and mables_out!=[]:
                    output_str += '\t'.join(line[0:-1])+'\t'+'|'.join(mables_out)
                    output_str += '\n'
                elif csets_out!=[] and mables_out==[]:
                    output_str += '\t'.join(line[0:-1])+'\t'+'|'.join(csets_out)
                    output_str += '\n'
                else:
                    output_str += '\t'.join(line[0:-1])+'\t'+'|'.join(csets_out)+'|'+'|'.join(mables_out)
                    output_str += '\n'
