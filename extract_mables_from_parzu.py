...
def main(infile):
            ...
            for tok in sentence:
                ...
                if not tok[-1].endswith('-') and not tok[-1].endswith('_'): #and not tok[4]=='PRF': #no reflexives
                    for id in tok[-1].split('|'):
                        ...
                        if re.match('\(\d+\)',id):
                            if cid in coref: coref[cid].append([sent_nr,int(tok[0]),int(tok[0])])
                            ...
                            for ext in aggr:
                                if ext[0]==cid:
                                    ...
                                    break
                            if cid in coref: coref[cid].append(ext)
                            else: coref[cid]=[ext]
                        else: pdb.set_trace()
    ...
    output_str += 'mables=' + repr(mables2) + '\n'
    output_str += 'coref=' + repr(coref) + '\n'
    ...
    output_str += 'str_matches=' + repr(str_matches2) + '\n'
    output_str += 'pposat_heads=' + repr(pposat_heads) + '\n'
    output_str += 'nominal_mods=' + repr(dict(nominal_mods)) + '\n'
    output_str += 'verbs=' + repr(all_verbs) + '\n'
    output_str += 'preds=' + repr(preds) + '\n'
    output_str += 'haben=' + repr(haben) + '\n'
    output_str += 'gmods=' + repr(gmods) + '\n'
    #print 'sentences=',sentences
    output_str += 'definite=[]' + '\n'
    output_str += 'demonstrative=[]' + '\n'
    output_str += 'determiners=' + repr(determiners) + '\n'
    output_str += 'prepositions=' + repr(prepositions) + '\n'
    output_str += '####' + '\n'
    #sent_nr-=1                 
